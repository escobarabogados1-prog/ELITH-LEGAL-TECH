  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] text-slate-900 font-sans">
      ...
      <main className="max-w-6xl mx-auto p-6 md:p-12">
        ...
        {view === 'proteccion' && (
          <div className="max-w-4xl mx-auto space-y-10 text-left">
            ...
            <div className="p-8 bg-slate-900 text-white rounded-[3rem] space-y-6 italic">
              <p className="text-sm font-light">Blindaje patrimonial absoluto: los bienes ingresan al patrimonio del fiduciario garantizando seguridad total contra embargos.</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                <div className="bg-slate-800 p-4 rounded-2xl"><p className="text-emerald-400 text-[10px] font-black">Fideicomitente</p></div>
                <div className="bg-slate-800 p-4 rounded-2xl"><p className="text-emerald-400 text-[10px] font-black">Fiduciario</p></div>
                <div className="bg-slate-800 p-4 rounded-2xl"><p className="text-emerald-400 text-[10px] font-black">Beneficiario</p></div>
              </div>
            </div>
            <button onClick={() => setView('pago')} className={btnStyle}>Iniciar Blindaje de Bienes</button>
          </div>
        )}
        ...
        {view === 'marcas' && (
          <div className="max-w-4xl mx-auto space-y-10 text-left">
            <button onClick={() => setView('home')} className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400"><Icon.ArrowLeft size={14}/> Volver</button>
            <h2 className="text-5xl font-black italic uppercase">Marcas <span className="text-emerald-500">SIC</span></h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {["Registro en 10 min", "Sin complicaciones", "Abogados Especialistas", "Vigilancia por 10 años", "Asistencia 24/7", "Pago en línea"].map((m,i) => (
                <div key={i} className="p-5 bg-white border border-slate-100 rounded-2xl flex items-center gap-3 italic font-bold text-xs"><Icon.Zap className="text-emerald-500" size={18}/> {m}</div>
              ))}
            </div>
            <button onClick={() => setView('pago')} className={btnStyle}>Solicitar Registro en Línea</button>
          </div>
        )}
        ...
        {view === 'pago' && (
          <div className="max-w-lg mx-auto p-12 border-2 border-emerald-500 rounded-[4rem] bg-white shadow-2xl text-center space-y-10">
            ...
            <button className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase text-xs flex items-center justify-center gap-2 shadow-lg hover:bg-blue-700">
              <Icon.Globe size={18}/> PSE / Mercado Pago
            </button>
            ...
            <button onClick={() => setView('carga-docs')} className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-emerald-600">Ya pagué, cargar documentos</button>
          </div>
        )}

        {/* VISTA: CARGA DOCUMENTOS */}
        {view === 'carga-docs' && (
          <div className="max-w-2xl mx-auto p-12 border-4 border-dashed border-slate-100 rounded-[4rem] text-center space-y-8">
            <h2 className="text-4xl font-black uppercase italic tracking-tighter">Notaría <span className="text-emerald-500">Digital</span></h2>
            <div className="grid grid-cols-2 gap-4">
              <div className="p-12 bg-slate-50 rounded-3xl border flex flex-col items-center gap-3 hover:border-emerald-500 transition-all cursor-pointer">
                <Icon.FileText className="text-emerald-500" size={40}/>
                <span className="text-[10px] font-black uppercase italic">Expediente</span>
              </div>
              <div className="p-12 bg-slate-50 rounded-3xl border flex flex-col items-center gap-3 hover:border-emerald-500 transition-all cursor-pointer">
                <Icon.CreditCard className="text-emerald-500" size={40}/>
                <span className="text-[10px] font-black uppercase italic">Soporte Pago</span>
              </div>
            </div>
            <button onClick={() => alert("Documentos enviados satisfactoriamente.")} className={btnStyle}>Finalizar Radicación</button>
          </div>
        )}
      ...
      <footer className="py-20 opacity-30 text-[9px] font-black uppercase tracking-[0.4em] italic text-center">
        © 2025 Elith Lex Group · Abogados +10 Años · Bogotá, Colombia.
      </footer>
    </div>
  );
}
